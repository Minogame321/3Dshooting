...
void drawospreybody(){
  ...
  popMatrix();
	...
	pushMatrix();
  	...
  	textureMode(NORMAL);
  		beginShape();
  			texture(img1);
        vertex(0,0,0,0,0); vertex(0,0,50,1,0); vertex(50*sqrt(3)/2,0,25,0.5,0.5);
      endShape();
  ...
  pushMatrix();
      ...
      beginShape();
        texture(img1);
        vertex(0,0,0,0,0); vertex(-42,-15,-12,0.3,0.3); vertex(-130,0,-25,0.5,0.5); vertex(-42,-15,-38,0.8,0.3); vertex(0,0,-50,1,0);
      endShape();

      beginShape();
        texture(img1);
        vertex(0,0,0,0,0); vertex(-42,-15,-12,0.3,0.3); vertex(-130,0,-25,0.4,0);  vertex(0,-34,0,1,0);
      endShape();
    ...
    translate(0,-34,50);
      ...
      beginShape();
        texture(img1);
        vertex(0,0,0,0,0);  vertex(-130,35,-25,0.5,0.5);  vertex(0,0,-50,1,0);
      endShape();
...
int score=0;
		...
		void draw(){
		  ...
		  popMatrix();
			...
			for(int i=0;i<10;i++){
				for(int j=0;j<7;j++){
					if(pow(b[i].getx()-e[j].getx(),2)+pow(b[i].gety()-e[j].gety(),2)+pow(b[i].getz()-e[j].getz(),2)<pow(e[j].getr()+4.5,2)){e[j].dead(); }
				}
		  }
			for(int i=0;i<7;i++){
				if(pow(eb[i].getx()-map(mouseX, 0, width, -125, 125),2)+pow(eb[i].gety()-map(mouseY, 0, height, -65, 65)+100,2)+pow(eb[i].getz()-120,2)<100 &&death==0){
					life-=5;
					if(life<0 &&life!=-0.001){life=-0.001; death=1; dx=map(mouseX, 0, width, -125, 125); dy=map(mouseY, 0, height, -65, 65)-100;}
				}
			}
